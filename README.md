...
2. 左側 **SQL Editor** → New query → 把 `supabase/schema.sql` 整份貼進去 → Run。
   接著再開一個 New query,把 `supabase/migration-02-listings.sql` 也貼進去 Run(車輛刊登、使用者代碼、收緊權限)。
...
3. 在「市場」分頁點右上角 **+** 各自刊登一台車,然後互相點對方車輛的「聯絡賣家」。
   或到「訊息」分頁點 **+**,輸入對方的六碼代碼。
...
**從車輛刊登** — 在市場點「聯絡賣家」。對話會綁定那則刊登,標籤自動是「買家詢問」,列表和對話框都會顯示「關於 2020 山葉 勁戰六代」。同一則刊登、同一組買賣雙方只會有一個對話,不會重複開。

**用使用者代碼** — 每個帳號註冊時會拿到一組六碼代碼(像 LINE ID),在訊息中心點 + 輸入對方代碼或註冊信箱。找不到人就不會建立對話。自己的代碼在同一個面板下方,點一下可以複製。

沒有「瀏覽全站使用者」這種入口。`profiles` 的 RLS 也一併收緊了,現在只看得到自己、有共同對話的人、以及架上有刊登的賣家 —— 就算直接打 API 也撈不到全站名單。
...
等組員都註冊完,到 SQL Editor 跑:

```sql
select public.seed_demo_listings();
```

會建六筆示範車輛,平均分給目前所有帳號,這樣每個人手上都有車可以被聯絡。重跑會先清掉舊的示範資料,不影響大家自己刊登的車。

## 還沒做的

搬回主專案前需要補的:訊息分頁(目前一次撈 200 則)、圖片訊息、封鎖與檢舉、推播通知、送出頻率限制、對方正在輸入的提示。

信箱查詢那條路會透露「這個信箱有沒有註冊過」。內部測試無所謂,對外開放前建議只留代碼查詢。
